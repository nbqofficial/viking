...
// Export the API symbols from this TU only
#define DLL_EXPORT
#include "nnue.h"
#undef DLL_EXPORT
...
// Old gcc on Windows is unable to provide a 32-byte aligned stack with AVX2.
...
static INLINE int16x8_t vmovl_high_s16(int8x16_t v) { return vmovl_s16(vget_high_s16(v)); }
#endif
...
static uint32_t PieceToIndex[2][14] = {
...
enum { kHalfDimensions = 256, FtInDims = 64 * PS_END, FtOutDims = kHalfDimensions * 2 };
...
typedef struct { size_t size; unsigned values[30]; } IndexList;

static INLINE int orient(int c, int s) { return s ^ (c == 0 ? 0x00 : 0x3f); }
static INLINE unsigned make_index(int c, int s, int pc, int ksq) {
...
// -------------------- Forward declaration (single canonical signature) --------------------
static INLINE void affine_txfm(
    clipped_t* input,
    void* output,
    unsigned             inDims,
    unsigned             outDims,
...
static INLINE int32_t affine_propagate(clipped_t* input, int32_t* biases, weight_t* weights) {
#if defined(USE_AVX2)
...
inline unsigned int portable_ctzll(uint64_t value) {
    unsigned long index;
...
#ifdef VECTOR
static INLINE bool next_idx(unsigned* idx, unsigned* offset, mask2_t* v, mask_t* mask, unsigned inDims) {
    while (*v == 0) {
...
// -------------------- affine_txfm implementations (same signature) --------------------
#if defined(USE_AVX512)
INLINE void affine_txfm(
    clipped_t* input,
    void* output,
    unsigned       inDims,
    unsigned       outDims,
    ...
    bool           pack8_and_calc_mask)
{
    ...
    for (unsigned offset = 0; offset < inDims;) {
        ...
        if (next_idx(&idx, &offset, &v, inMask, inDims)) {
            ...
            factor |= ((uint16_t)(uint8_t)input[idx]) << 8;
        }
        else {
            second = kZero;
        }
    ...
    if (pack8_and_calc_mask) outMask[0] = (uint32_t)_mm256_movemask_epi8(_mm256_cmpgt_epi8(outVec[0], kZero256));
    else                     outVec[0] = _mm256_max_epi8(outVec[0], kZero256);
}
#elif defined(USE_AVX2)
INLINE void affine_txfm(
    clipped_t* input,
    ...
    bool           pack8_and_calc_mask)
{
    ...
    if (pack8_and_calc_mask) outMask[0] = _mm256_movemask_epi8(_mm256_cmpgt_epi8(outVec[0], kZero));
    else                     outVec[0] = _mm256_max_epi8(outVec[0], kZero);
}
#elif defined(AVOID_USE_SSSE3) || defined(USE_SSE2)
INLINE void affine_txfm(
    clipped_t* input,
    void* output,
    unsigned       inDims,
    ...
    bool           pack8_and_calc_mask)
{
    ...
    const __m128i* first; const __m128i* second;
    mask2_t v; unsigned idx;

    memcpy(&v, inMask, sizeof(mask2_t));
    for (unsigned offset = 0; offset < inDims;) {
        if (!next_idx(&idx, &offset, &v, inMask, inDims)) break;
        first = (__m128i*) & weights[32 * idx];
        uint32_t factor = (uint8_t)input[idx];
        if (next_idx(&idx, &offset, &v, inMask, inDims)) {
            second = (__m128i*) & weights[32 * idx];
            factor |= ((uint32_t)(uint8_t)input[idx]) << 16;
        }
        else {
            second = kZeros;
        }
    ...
    if (pack8_and_calc_mask) {
        ...
        const __m128i kMax = _mm_set1_epi16(127);
        outVec[0] = _mm_min_epi16(_mm_max_epi16(out16_0, kZero), kMax);
        outVec[1] = _mm_min_epi16(_mm_max_epi16(out16_1, kZero), kMax);
        outVec[2] = _mm_min_epi16(_mm_max_epi16(out16_2, kZero), kMax);
        outVec[3] = _mm_min_epi16(_mm_max_epi16(out16_3, kZero), kMax);
    }
#endif
}
#elif defined(USE_MMX)
    ...
    bool           pack8_and_calc_mask)
{
    ...
    const __m64* first; const __m64* second;
    mask2_t v; unsigned idx;
    ...
    bool           pack8_and_calc_mask)
{
    ...
    const int8x8_t* first; mask2_t v; unsigned idx;

    memcpy(&v, inMask, sizeof(mask2_t));
    for (unsigned offset = 0; offset < inDims;) {
        if (!next_idx(&idx, &offset, &v, inMask, inDims)) break;
        first = (int8x8_t*)&weights[32 * idx];
        int16_t factor = (uint8_t)input[idx];
    ...
    bool           pack8_and_calc_mask)
{
    ...
    for (unsigned idx = 0; idx < inDims; ++idx) {
        if (input[idx]) {
            const weight_t* w = &weights[size_t(outDims) * idx];
            int32_t x = static_cast<int8_t>(input[idx]);
            for (unsigned i = 0; i < outDims; ++i) tmp[i] += x * static_cast<int8_t>(w[i]);
        }
    }

    clipped_t* outVec = static_cast<clipped_t*>(output);
...
static INLINE void refresh_accumulator(Position* pos) {
    Accumulator* accumulator = &(pos->accumulator);
    IndexList activeIndices[2]; activeIndices[0].size = activeIndices[1].size = 0;
    ...
    for (unsigned c = 0; c < 2; c++) {
        ...
        for (size_t k = 0; k < activeIndices[c].size; k++) {
            unsigned index = activeIndices[c].values[k];
            unsigned off = kHalfDimensions * index;
            for (unsigned j = 0; j < kHalfDimensions; j++)
                accumulator->accumulation[c][j] += ft_weights[off + j];
        }
#endif
    }
...
static INLINE void transform(Position* pos, clipped_t* output, mask_t* outMask) {
    refresh_accumulator(pos);
    int16_t(*acc)[2][256] = &pos->accumulator.accumulation;
    (void)outMask;
...
// Public function used by your board code
int nnue_evaluate_pos(Position* pos) {
    ...
    uint8_t raw[sizeof(struct NetData) + 63];
    struct NetData* b = (struct NetData*)(raw + ((((uintptr_t)raw - 1) ^ 0x3f) & 0x3f));
#  define B(x) (b->x)
#else
    struct NetData buf;
#  define B(x) (buf.x)
...
// --------- weight loading / file mapping ---------

static INLINE void read_output_weights(weight_t* w, const char* d) {
    for (unsigned i = 0; i < 32; i++) w[i] = *d++;
}

static INLINE unsigned wt_idx(unsigned r, unsigned c, unsigned dims) {
#if defined(USE_AVX512)
    if (dims > 32) { unsigned b = c & 0x38; b = (b << 1) | (b >> 2); c = (c & ~0x38) | (b & 0x38); }
    else if (dims == 32) { unsigned b = c & 0x18; b = (b << 1) | (b >> 1); c = (c & ~0x18) | (b & 0x18); }
#elif defined(USE_AVX2)
    if (dims > 32) { unsigned b = c & 0x18; b = (b << 1) | (b >> 1); c = (c & ~0x18) | (b & 0x18); }
#endif
#if defined(USE_AVX512)
    return c * 64 + r + (r & ~7);
#else
...
static void permute_biases(int32_t* biases) {
    __m128i* b = (__m128i*)biases; __m128i tmp[8];
    tmp[0] = b[0]; tmp[1] = b[4]; tmp[2] = b[1]; tmp[3] = b[5]; tmp[4] = b[2]; tmp[5] = b[6]; tmp[6] = b[3]; tmp[7] = b[7];
...
static bool load_eval_file(const char* evalFile) {
    const void* evalData; map_t mapping; size_t size;
#ifdef NNUE_EMBEDDED
    if (strcmp(evalFile, DefaultEvalFile) == 0) {
        evalData = gNetworkData; mapping = 0; size = gNetworkSize;
    }
    else
#endif
    {
    ...
    bool ok = verify_net(evalData, size);
    if (ok) init_weights(evalData);
    if (mapping) unmap_file(evalData, mapping);
    return ok;
}

// -------------- Public C API (matches nnue.h) --------------
static char* loadedFile = NULL;

void NNUE_CALL nnue_init(const char* evalFile) {
    if (loadedFile && strcmp(evalFile, loadedFile) == 0) return;
    if (loadedFile) free(loadedFile);

    printf("Loading NNUE : %s\n", evalFile); fflush(stdout);
    if (load_eval_file(evalFile)) {
        loadedFile = _strdup ? _strdup(evalFile) : strdup(evalFile);
        printf("NNUE loaded !\n"); fflush(stdout);
        return;
    }
...
int NNUE_CALL nnue_evaluate(int player, int* pieces, int* squares) {
    Position pos; pos.player = player; pos.pieces = pieces; pos.squares = squares;
    return nnue_evaluate_pos(&pos);
}

int NNUE_CALL nnue_evaluate_fen(const char* fen) {
